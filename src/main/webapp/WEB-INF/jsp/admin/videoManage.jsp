...
<head>
    ...
    <link href="/assert/css/base.css" type="text/css" rel="stylesheet">
    <link href="/assert/css/tab.css" type="text/css" rel="stylesheet">
    <link href="/assert/css/item.css" type="text/css" rel="stylesheet">
    <link href="/assert/css/item_do.css" type="text/css" rel="stylesheet">
    <link rel="stylesheet" href="/assert/css/uploadify.css" type="text/css"></link>
    ...
    <script type="text/javascript" src="/assert/js/jquery.uploadify.v2.0.3.js"></script>
    <script type="text/javascript" src="/assert/js/swfobject.js"></script>
        <%
	String type = request.getParameter("type");
	String grade = request.getParameter("grade");
%>
<body style="margin:1px;" id="ff">
<table id="dg" title="视频信息管理" class="easyui-datagrid" pagination="true"
       rownumbers="true" fit="true" data-options="pageSize:10"
       url="${pageContext.request.contextPath}/videos/datagrid?type=<%=type %>&grade=<%=grade %>"
       toolbar="#tb">
    <thead data-options="frozen:true">
    <tr>
        <th field="cb" checkbox="true" align="center"></th>
        <th field="id" width="10%" align="center" hidden="true">编号</th>
        <th field="path" width="300" align="center" formatter="formatProPic">视频预览</th>
        <th field="time" width="150" align="center">创建时间</th>
        <th field="url" width="150" align="center">视频链接</th>
    </tr>
    </thead>
</table>
<div id="tb">
    <div>
        <a href="javascript:openVideoAddDialog()" class="easyui-linkbutton"
           iconCls="icon-add" plain="true">添加</a> <a
            href="javascript:openVideoModifyDialog()"
    ...
    <div>
        &nbsp;标题：&nbsp;<input type="text" id="url" size="20"
                              onkeydown="if(event.keyCode==13) searchVideo()"/>&nbsp; <a
            href="javascript:searchVideo()" class="easyui-linkbutton"
    ...
    <form id="fm" method="post" enctype="multipart/form-data">
        <div style="padding-top:50px;  float:left; width:95%; padding-left:30px;">
            <div id="i_do_wrap">
                ...
                <div class="i_do_div rel" id="video"><p class="i_do_tle r_txt abs font14">展示视频</p>
                </div>
                <div class="i_do_div rel" id="i_no_sku_stock_wrap"><p class="i_do_tle r_txt abs font14">视频链接</p>
                    <input type="text" id="videoUrl" name="url" value="" required="true" class="easyui-validatebox"
                           style="border:1px #9c9c9c solid;height:25px;"/>
                    <input type="hidden" id="videoType" name="type" value="<%=type%>"/>
                    <input type="hidden" id="videoGrade" name="grade" value="<%=grade%>"/>
                    <input id="videoId" name="id" type="hidden" value="0">
...
<div id="dlg-buttons">
    <a href="javascript:saveVideo()" class="easyui-linkbutton"
       iconCls="icon-ok">保存</a> <a href="javascript:closeVideoDialog()"
...
<script type="text/javascript">
    var url = "${pageContext.request.contextPath}/videos";
    var method;
    ...
    function saveVideo() {
        ...
        var grade = $("#videoGrade").val();
        var id = $("#videoId").val();
        var videoUrl = $("#videoUrl").val();
        console.log(path);
        var data = {"id": id, "grade": grade, "type": type, "url": videoUrl, "path": path}
        $.ajax({
            type: method,//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: url,//url
    ...
    function initUploadify() {
        $("#uploadify2").uploadify({
            ...
            'fileDesc': '*.mp4', 	//用来设置选择文件对话框中的提示文本
            'fileExt': '*.mp4', 		//设置可以选择的文件的类型
            'auto': true, 								//设置为true当选择文件后就直接上传了，为false需要点击上传按钮才上传
            'multi': true, 								//设置为true时可以上传多个文件
            'simUploadLimit': 1, 						//允许同时上传的个数 默认值：1
            'sizeLimit': 20480000,						//上传文件的大小限制
            'buttonText': '上传视频',						//浏览按钮的文本，默认值：BROWSE
